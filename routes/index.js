...
function update_last_version(){
  ...
  if (local_version < remote_version) {
    console.log('OUT OF DATE VERSION.');
    console.log('UPDATING VERSION TO ' + execute("npm show venom-bot version").split('\n')[0] + '.');
    //return execute("npm update venom-bot");
    execute("npm install venom-bot@" + remote_version);
    execute("npm fund");
  }else{
    console.log('IT IS ALREADY WITH THE LATEST VERSION.');
  }
}
  ...
  async function check_user() {
    ...
    if (!user) {
      ...
      User.comparePassword(req.body.password, user, (err, isMatch) =>{
        ...
        return res.json({sucess: true, message: "Login successfully.", key: user.session_key, user_id: user.id, is_logged: is_logged});
      });
...
router.get('/database/reset', function (req, res) {
  ...
  User.destroy({
    ...
    if(deletedRecord){
      res.status(200).json({success: true, message:"Database successfully reset !!!"});
    } else {
      res.status(404).json({success: false, message:"Error when resetting database."})
    }
  }).catch(function (error) {
    res.status(500).json(error);
  })
...
router.get('/database/load', function (req, res) {
  User.findAll().then(function (load_data) {
    if(load_data){
      res.status(200).json({success: true, object: load_data, message:"Database loaded successfully !!!"});
    } else {
      res.status(404).json({success: false, message:"Error loading database."})
    }
  }).catch(function (error) {
    res.status(500).json(error);
  })
...
router.get("/new_qrcode", async function (req, res, next) {
  ...
  if (session) {
    ...
    console.log('LOOK AT THE STATUS: ' + session.status + ' AND LOOK AT THE STATE: ' + session.state);

    if (session.status === 'qrReadSuccess'){
      return res.json({success: 'true', object: false, message: 'You are already logged in!!!', is_active: true});
    }
    if (session.status === 'isLogged' || session.status === 'inChat'){
      //res.redirect('/testing');
      return res.json({success: 'true', object: false, message: 'You are already logged in!!!', is_active: true});
    }else if (session.status === 'notLogged' && session.state !== 'STARTING') {
      res.json({success: 'true', object: session.qrcode, message: 'New qrcode successfully generated!!!', is_active: false});
    }
  } else {
